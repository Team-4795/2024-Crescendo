...
public class NoteVisualizer {
  ...
  private static final double shotSpeed = 8.0; // Meters per sec
        ...
        Commands.defer(
                () -> {
                   ...
                   final Pose3d startPose = new Pose3d(
                      robotPoseSupplier.get().getX() + 0.8 * Math.cos(Pivot.getInstance().getPosition()) - 0.5,
                      robotPoseSupplier.get().getY(),
                      Math.abs(0.8 * Math.sin(Pivot.getInstance().getPosition())),
                      new Rotation3d(0, (isRed ? -1 : 1) *Pivot.getInstance().getPosition(), 0));
