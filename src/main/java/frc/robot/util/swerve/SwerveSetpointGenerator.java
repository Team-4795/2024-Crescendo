...
public class SwerveSetpointGenerator {
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    if (Util.epsilonEquals(Util.toTwist2d(desiredState), new Twist2d())) {
      need_to_steer = false;
    ...
    if (all_modules_should_flip
        && !Util.epsilonEquals(Util.toTwist2d(prevSetpoint.chassisSpeeds()), new Twist2d())
        && !Util.epsilonEquals(Util.toTwist2d(desiredState), new Twist2d())) {
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      if (Util.epsilonEquals(prevSetpoint.moduleStates()[i].speedMetersPerSecond, 0.0)) {
